...
in
{
  ...
  programs.zsh = {
    ...
    initContent = lib.mkForce (
      ''
        ...
        [[ -r "${config.home.homeDirectory}/.config/secrets/claude-code/env.sh" ]] && \
          source "${config.home.homeDirectory}/.config/secrets/claude-code/env.sh"
      ''
  ...
  home.file."Library/Application Support/iTerm2/DynamicProfiles/dream.json" = {
    text = lib.replaceStrings [ "/Users/dream" ] [ config.home.homeDirectory ] (
      builtins.readFile ./terminal/iterm2-profile.json
    );
  };
  ...
  programs.fish = {
    ...
    shellInit = builtins.readFile ./shell/init.fish + ''
      ...
      if test -r "${config.home.homeDirectory}/.config/secrets/claude-code/env.fish"
        source "${config.home.homeDirectory}/.config/secrets/claude-code/env.fish"
      end
    '';
  };
